...
richness_env$pos <- numFactor(coordinates(sp_station)[,1], coordinates(sp_station)[,2])
...
dfids <- subset(assos_mat, sp1_name == sp2_name) %>%
  ...
  mutate(color_sp = ifelse(dfids$X1 %in% big_sp, "red", "green")
  )
...
assos_mat <- cooc %>%
  select(sp1_name, sp2_name, assos) %>%
...
assos_mat_vincent_strength <- cooc_bivalve$results %>%
  ...
  select(sp1_name, sp2_name, strength) %>%
