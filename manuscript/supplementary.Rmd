...
title(xlab = "Species richness", ylab = "Spatially lagged species richness")
invisible(dev.off())
```

```{r moran, fig.cap="Moran plot. Spatially lagged sites shows correlation."}
#include_graphics(fig_moran_path)
...
title(ylab = "Model residuals", xlab = "Prediction of the model")
invisible(dev.off())
```
...
```
```{r, fig.cap=resid_cap}
#include_graphics(fig_resid_path)
